...
- **`grid.ready()` resolves before Angular applies `gridConfig`** — The web component's `ready()` promise resolves from `connectedCallback()`'s synchronous render. Angular's Grid directive applies `gridConfig` asynchronously via `effect()` + `queueMicrotask`, which means plugins (filtering, selection, etc.) are attached *after* `ready()` has already resolved. Inject functions (`injectGridFiltering`, etc.) must check for the plugin's existence after `ready()` and defer `isReady` via `setTimeout(0)` if the plugin isn't found yet. This lets pending microtasks flush before signaling readiness.
