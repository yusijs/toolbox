...
Manage selection state externally via the selection feature API:
...
The grid automatically handles virtualization — only visible rows are in the DOM. If you display the full height of the grid, virtualization is bypassed.
...
export class EmployeeGridComponent {
  ...
  gridConfig: GridConfig<Employee> = {
    ...
    features: {
      serverSide: { dataSource: this.dataSource },
    },
  };
}
```
...
Use the responsive feature with the Angular `GridResponsiveCard` directive for mobile-friendly card layouts:
