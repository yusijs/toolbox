...
:::caution
Do not place `<style>` elements as children of `<tbw-grid>` — the grid calls `replaceChildren()` during renders, which removes them. Use `registerStyles()` for styles that must be injected at runtime. External stylesheets (e.g., `<link>` tags, theme CSS files, or `<style>` in `<head>`) are unaffected.
:::
