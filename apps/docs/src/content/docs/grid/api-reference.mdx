...
Icons are rendered via **CSS custom properties** by default — no JavaScript configuration needed. Override any icon with CSS:
...
@layer tbw-theme {
  tbw-grid {
    ...
    /* SVG mask icons — set text content to '' and provide a mask URL */
    --tbw-icon-filter: '';
...
For programmatic control (e.g., dynamic icon libraries), use `gridConfig.icons` which takes precedence over CSS:
