...
// Note: ctx.renderSortIcon() returns a DOM element.
// To include it in React JSX, use a ref or dangerouslySetInnerHTML,
// or return the full DOM element from the renderer instead:
{
  field: 'email',
  headerRenderer: (ctx) => {
    const wrapper = document.createElement('div');
    wrapper.style.cssText = 'display:flex;align-items:center;gap:6px;width:100%';
    wrapper.append('📧 ', ctx.value);
    const sortIcon = ctx.renderSortIcon();
    if (sortIcon) wrapper.appendChild(sortIcon);
    return wrapper; // Returning DOM elements also works in React config
  },
}
```
  </TabItem>
...
// Note: ctx.renderSortIcon() returns a DOM element.
// For sort icon support, use a vanilla DOM renderer instead:
{
  field: 'email',
  headerRenderer: (ctx) => {
    const wrapper = document.createElement('div');
    ...
    const sortIcon = ctx.renderSortIcon();
    if (sortIcon) wrapper.appendChild(sortIcon);
    return wrapper; // Returning DOM elements also works in Vue config
  },
}
```
  </TabItem>
...
:::tip
The framework adapters bridge `loadingRenderer` — you can use React JSX, Vue components/render functions, or Angular component classes. Vanilla DOM functions also work. Pass the config via `gridConfig` in React (`<DataGrid gridConfig={config}>`), Vue (`:grid-config="config"`), and Angular (`[gridConfig]="config"`).
:::
