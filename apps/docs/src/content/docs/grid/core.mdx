...
| `initialSort` | `{ field, direction }` | Sort on first render (`'asc'` or `'desc'`) |
...
The renderer receives a [`CellRenderContext`](/grid/api/core/interfaces/cell-render-context) with these properties:

| Property | Type | Description |
|----------|------|-------------|
| `value` | `TValue` | The cell's current value |
| `row` | `TRow` | The full row data object |
| `field` | `string` | The column's field name |
