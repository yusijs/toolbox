...
import type { SelectionChangeDetail } from '@toolbox-web/grid';

function EmployeeGrid({ employees }) {
  const gridRef = useGrid<Employee>();

  useGridEvent(gridRef, 'selection-change', () => {
...
// Track dirty state
grid.addEventListener('dirty-change', () => {
...
function EditableGrid({ employees }) {
  ...
  const handleCellCommit = useCallback((detail: CellCommitDetail<Employee>) => {
    if (detail.field === 'email' && !detail.value.includes('@')) {
      detail.preventDefault(); // Reject invalid edit
    }
  }, []);
...
<CardGrid>
  <LinkCard title="Plugins Overview" description="Browse all 22+ plugins" href="/grid/plugins/" />
