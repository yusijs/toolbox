...
> **Requires:** `ServerSidePlugin` — [see Server-Side Data Loading guide](/grid/guides/server-side-data/)
...
watch(isReady, (ready) => {
  if (!ready) return;
  gridElement.value?.getPluginByName('serverSide')?.setDataSource({
    async getRows(params) {
      const res = await fetch(`/api/groups?start=${params.startNode}&end=${params.endNode}`);
      return res.json();
    },
...
<template>
  <TbwGrid :server-side="{ pageSize: 50 }" grouping-rows>
    <TbwGridColumn field="name" header="Employee" />
    <TbwGridColumn field="department" header="Department" />
    <TbwGridColumn field="salary" header="Salary" type="currency" />
  </TbwGrid>
