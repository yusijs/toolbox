...
```typescript
import '@toolbox-web/grid/features/all';
```
...
These recipes cover the most common grid scenarios using the features API. Dependencies are resolved automatically.
...
grid.gridConfig = {
  columns: [...],
  features: {
    selection: { mode: 'cell', multiSelect: true },
    editing: { mode: 'cell', dirtyTracking: true },
    undoRedo: true,
    clipboard: true,
  },
};
...
grid.gridConfig = {
  ...
  features: {
    selection: 'row',
    filtering: { filterHandler: async (model) => fetchFiltered(model) },
    multiSort: true,
    serverSide: { pageSize: 100 },
  },
};
```
...
grid.gridConfig = {
  columns: [...],
  features: {
    groupingRows: { groupOn: (row: any) => [row.department], defaultExpanded: true },
    selection: 'row',
    filtering: true,
  },
};
