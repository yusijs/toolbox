...
Use the controls to explore the full pivot configuration — toggle active state, totals, grand total, tool panel, default expansion, indent width, aggregation function, sorting, value formatting, and animation style.
...
Sort pivot rows and columns via configuration:
...
:::note
Pivot sorting is applied at build time via configuration. Interactive sorting by clicking column
headers is not currently supported — pivot columns are generated dynamically and don't participate
in the grid's built-in sort mechanism.
:::
